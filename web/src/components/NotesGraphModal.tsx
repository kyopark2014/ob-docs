...
import { api, type NotesGraphStatus } from "../api";
...
export function NotesGraphModal({
  title = "Notes Graph",
  onClose,
  onOpenNote,
}: Props) {
  const [status, setStatus] = useState<NotesGraphStatus | null>(null);
  const [frameSrc, setFrameSrc] = useState<string | null>(null);
  const [pollError, setPollError] = useState<string | null>(null);
  const [patternBusy, setPatternBusy] = useState(false);
  const patternBusyRef = useRef(false);

  const applyStatus = useCallback((next: NotesGraphStatus) => {
    setStatus(next);
    if (next.exists) {
      setFrameSrc((prev) => {
  ...
  useEffect(() => {
    async function onMessage(e: MessageEvent) {
      ...
      if (data.type === "notes-open" && typeof data.path === "string") {
        onOpenNote?.(data.path);
        onClose();
        return;
      }
      if (data.type !== "graph-pattern") return;
  ...
  return createPortal(
    <div
      className="modal-backdrop knowledge-graph-modal"
      role="dialog"
      aria-modal="true"
      aria-label={title}
      onClick={(e) => {
      ...
      <div className="knowledge-graph-panel">
        ...
        {showFrame ? (
          ...
          <div className="knowledge-graph-placeholder">
            {busy ? (
              ...
              <>
                <p className="knowledge-graph-placeholder-title">Notes Graph 없음</p>
                <p className="knowledge-graph-placeholder-body">
                  좌측 Graph → Sync로 먼저 동기화하세요
                  {status?.notes_dir ? ` (${status.notes_dir})` : ""}.
                </p>
              </>
            )}
          </div>
        ...
        {showFrame && (busy || patternBusy) ? (
          <div className="knowledge-graph-banner">
            {patternBusy ? "패턴 전환 중…" : "Notes 동기화 중…"}
          </div>
        ) : null}
      </div>
    </div>,
