#!/usr/bin/env python3
"""Register an additional Cognito user for ob-note Web UI login.

Reads User Pool / App Client settings from config.json (written by
...
def load_cognito_config(config_path: Path) -> Dict[str, str]:
    if not config_path.is_file():
        raise FileNotFoundError(
            ...
            "(choose Cognito auth when google_client_id is missing)."
        )
    with config_path.open(encoding="utf-8") as f:
        config: Dict[str, Any] = json.load(f)

    auth_mode = (config.get("auth_mode") or "").strip().lower()
    ...
    if not user_pool_id or not client_id:
        raise ValueError(
            ...
            "Run installer.py and select Cognito auth "
            "(requires no google_client_id, or auth_mode=cognito)."
        )
    if auth_mode and auth_mode != "cognito":
        logger.warning(
            "config auth_mode=%s (expected cognito) — continuing with Cognito pool settings",
            auth_mode,
        )
    return {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Register a Cognito user for ob-note Web UI login.",
    )
    parser.add_argument(
        "--username",
        "-u",
...
def main() -> int:
    ...
    logger.info("  User Pool:  %s", user_pool_id)
